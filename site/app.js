...
function applyNav(){document.body.dataset.route=state.route;$$("[data-route]").forEach(a=>{const r=a.dataset.route;a.classList.toggle("active",r===state.route);a.classList.toggle("locked",!canRoute(r))});$("#accountName").textContent=state.actor?.email||"";const act=$("#mainNav a.active");if(act&&matchMedia("(max-width:980px)").matches)act.scrollIntoView({inline:"center",block:"nearest"})}
...
async function radio(){
 ...
 const pills='<div class="venue-pills" data-filter-group="radio"><button class="active" data-space="all">Todos</button>'+groups.map((v,i)=>'<button data-space="'+i+'">'+esc(v)+'</button>').join("")+'</div>';
 ...
 pills+'<div class="module-kpi"><span><b>'+active.length+'</b><em>En emisión ahora</em></span><span><b>'+rows.length+'</b><em>Campañas registradas</em></span></div>'+
 ...
 $$('[data-filter-group="radio"] button').forEach(b=>b.onclick=()=>{$$('[data-filter-group="radio"] button').forEach(x=>x.classList.toggle("active",x===b));const target=b.dataset.space;$$("[data-radio-space]").forEach(p=>p.classList.toggle("filtered-out",target!=="all"&&p.dataset.radioSpace!==target))});
 bindRadio(rows);
...
async function campaigns(moduleName){
 ...
 const filters='<div class="form-grid" style="margin-bottom:12px"><label>Buscar<input id="campaignFilterQ" placeholder="Espectáculo, soporte, proveedor…"></label><label>Estado<select id="campaignFilterStatus"><option value="">Todos</option><option value="active">Activas ahora</option><option value="finalizado">Finalizadas</option></select></label><label class="wide">Espacio<select id="campaignFilterVenue"><option value="">Todos los espacios</option>'+VENUES.map(v=>'<option>'+esc(v)+'</option>').join("")+'</select></label></div>';
 ...
 $("#campaignFilterQ").addEventListener("input",refresh);$("#campaignFilterStatus").addEventListener("input",refresh);$("#campaignFilterVenue").addEventListener("input",refresh);
...
async function homeTicket(){
 ...
 const pills='<div class="venue-pills" data-filter-group="ht"><button class="active" data-space="all">Todos los espacios</button>'+HOME_TICKET_SPACES.map((v,i)=>'<button data-space="'+i+'">'+esc(v.replace("Gran Teatro CaixaBank ","").replace("Gran Teatro ",""))+'</button>').join("")+'</div>';
 const grouped=HOME_TICKET_SPACES.map((v,i)=>'<div class="ht-space space-panel" data-ht-space="'+i+'"><div class="section-title"><div><small class="section-kicker">'+String(i+1).padStart(2,"0")+' · HOME TICKET</small><h2>'+esc(v)+'</h2></div><span class="badge">'+rows.filter(r=>r.venue===v).length+' piezas</span></div><div class="list">'+htItems(rows.filter(r=>r.venue===v))+'</div></div>').join("");
 app.innerHTML=pageHead("Home Ticket","Formatos y creatividades de los 5 espacios",'<button id="newHT" class="primary">+ Nueva pieza</button>')+pills+'<div class="module-kpi"><span><b>05</b><em>Espacios gestionados</em></span><span><b>'+rows.filter(activeNow).length+'</b><em>Piezas activas</em></span><span><b>'+rows.length+'</b><em>Piezas registradas</em></span></div>'+'<div class="grid two-col module-workspace"><section class="card">'+grouped+'</section><section class="card" id="htFormCard">'+htForm()+'</section></div>';
 $$('[data-filter-group="ht"] button').forEach(b=>b.onclick=()=>{$$('[data-filter-group="ht"] button').forEach(x=>x.classList.toggle("active",x===b));const target=b.dataset.space;$$("[data-ht-space]").forEach(p=>p.classList.toggle("filtered-out",target!=="all"&&p.dataset.htSpace!==target))});
