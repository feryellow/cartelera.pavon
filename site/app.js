...
function canRoute(route){if(roles().includes("admin"))return true;if(route==="dashboard"||route==="calendario"||route==="carteleria"||route==="archivo")return true;if((route==="radio"||route==="taxis"||route==="intercambiadores"||route==="hometicket")&&roles().includes("gestion"))return true;return false}
...
function pageHead(title,sub,actions=""){const home=state.route==="dashboard"?"":'<a class="btn back-home" href="#dashboard">← Inicio</a>';return '<div class="page-head"><div><h1>'+esc(title)+'</h1><p>'+esc(sub||"")+'</p></div><div class="actions-row">'+home+actions+"</div></div>"}
...
async function dashboard(){
 ...
 '<div class="grid stats">'+stat(alerts.length,"Avisos ≤ 7 días")+stat(d.radio?.active??"—","Radio activa")+stat(d.taxis?.active??"—","Taxis activos")+stat(d.intercambiadores?.active??"—","Intercambiadores activos")+'</div>'+
...
function venueSelect(v=""){const values=["Gran Teatro Pavón","Gran Teatro CaixaBank Príncipe Pío","Teatro Serrano","Gran Castillo de Pedraza","Abono Teatro","Soho City Madrid"];return '<label>Espacio<select name="venue"><option value="">Seleccionar…</option>'+values.map(x=>'<option '+(x===v?"selected":"")+'>'+esc(x)+'</option>').join("")+'</select></label>'}
...
function campaignItems(rows,moduleName){return rows.length?rows.map(r=>'<div class="item"><div><h3>'+esc(r.spectacle||r.campaignName||"Campaña")+'</h3><div class="item-meta"><span>'+esc(r.location||r.support||"")+'</span><span>'+esc(r.provider||"")+'</span><span>'+fdate(r.startDate)+' → '+fdate(r.endDate)+'</span>'+statusBadge(r.status)+'</div><div class="media-preview" data-asset="'+esc(r.assetKey||"")+'" data-module="'+moduleName+'" data-kind="image"></div></div><div class="item-actions"><button data-edit-campaign="'+r.id+'">Editar</button><button class="danger" data-del-campaign="'+r.id+'">Archivar</button></div></div>').join(""):'<div class="notice">No hay campañas registradas.</div>'}
function campaignForm(moduleName,r={}){const cfg=CAMPAIGN_CONFIG[moduleName];return '<div class="section-title"><h2>'+(r.id?"Editar campaña":"Nueva campaña")+'</h2></div><form id="campaignForm" class="form-grid">'+input("spectacle","Espectáculo / campaña",r.spectacle)+input("campaignName","Nombre interno",r.campaignName)+input("support","Soporte / formato",r.support)+input("location",cfg.location,r.location)+input("provider",cfg.provider,r.provider)+input("format","Pieza / formato",r.format)+input("startDate","Inicio",r.startDate,"date")+input("endDate","Fin",r.endDate,"date")+input("contact","Contacto",r.contact)+selectStatus(r.status)+'<label class="wide">Creatividad<input id="campaignAsset" type="file" accept="image/*,application/pdf"></label><label class="wide">Condiciones / acuerdo<textarea name="agreement">'+esc(r.agreement||"")+'</textarea></label><label class="wide">Observaciones<textarea name="notes">'+esc(r.notes||"")+'</textarea></label><div class="wide actions-row"><button class="primary" type="submit">Guardar</button><button type="button" id="cancelCampaign">Limpiar</button></div></form>'}
...
async function homeTicket(){
 ...
 app.innerHTML=pageHead("Home Ticket","HT Superior e Inferior 520 × 420 · XL 520 × 856",'<button id="newHT" class="primary">Nueva pieza</button>')+'<div class="grid two-col"><section class="card"><div class="section-title"><h2>Piezas</h2><span class="badge">'+rows.length+'</span></div><div class="list">'+htItems(rows)+'</div></section><section class="card" id="htFormCard">'+htForm()+'</section></div>';
 $("#newHT").onclick=()=>{$("#htFormCard").innerHTML=htForm();bindHTForm(null)};$$("[data-edit-ht]").forEach(b=>b.onclick=()=>{const r=rows.find(x=>x.id===b.dataset.editHt);$("#htFormCard").innerHTML=htForm(r);bindHTForm(r)});$$("[data-del-ht]").forEach(b=>b.onclick=async()=>{if(!confirm("¿Archivar esta pieza?"))return;await api("/api/control?module=hometicket&id="+b.dataset.delHt,{method:"DELETE"});say("Pieza archivada");homeTicket()});bindHTForm(null);await hydrateMedia("hometicket");
}
function htItems(rows){return rows.length?rows.map(r=>'<div class="item"><div><h3>'+esc(r.venue||"Espacio")+' · '+esc(r.position||"Home Ticket")+'</h3><div class="item-meta"><span>'+esc(r.spectacle||"")+'</span><span>'+fdate(r.startDate)+' → '+fdate(r.endDate)+'</span>'+statusBadge(r.status)+'</div><div class="media-preview" data-asset="'+esc(r.assetKey||"")+'" data-module="hometicket" data-kind="image"></div></div><div class="item-actions"><button data-edit-ht="'+r.id+'">Editar</button><button class="danger" data-del-ht="'+r.id+'">Archivar</button></div></div>').join(""):'<div class="notice">Todavía no hay piezas de Home Ticket cargadas.</div>'}
function htForm(r={}){return '<div class="section-title"><h2>'+(r.id?"Editar pieza":"Nueva pieza")+'</h2></div><form id="htForm" class="form-grid">'+venueSelect(r.venue)+'<label>Formato<select name="position">'+["HT Superior · 520 × 420","HT Inferior · 520 × 420","Home Ticket XL · 520 × 856"].map(x=>'<option '+(x===r.position?"selected":"")+'>'+x+'</option>').join("")+'</select></label>'+input("spectacle","Espectáculo / pieza",r.spectacle)+input("startDate","Inicio",r.startDate,"date")+input("endDate","Fin",r.endDate,"date")+selectStatus(r.status)+'<label class="wide">Creatividad<input id="htAsset" type="file" accept="image/*"></label><label class="wide">Observaciones<textarea name="notes">'+esc(r.notes||"")+'</textarea></label><div class="wide actions-row"><button class="primary" type="submit">Guardar</button><button type="button" id="cancelHT">Limpiar</button></div></form>'}
...
async function hydrateMediaasync function hydrateMedia(moduleName){for(const el of $$('[data-module="'+moduleName+'"][data-asset]')){const k=el.dataset.asset;if(!k)continue;const u=await blobUrl(k,moduleName);if(!u)continue;if(el.dataset.kind==="audio")el.innerHTML='<audio controls preload="none" src="'+u+'"></audio>';else el.innerHTML='<img src="'+u+'" alt="Creatividad">' }}

async function archivo(){
 const auditP=api("/api/control?module=audit&limit=250");
 const modules=(roles().includes("admin")||roles().includes("gestion"))?["radio","taxis","intercambiadores","hometicket"]:[];
 const results=await Promise.all([auditP,...modules.map(m=>api("/api/control?module="+m+"&includeDeleted=1"))]);
 const d=results[0],finished=[];
 modules.forEach((m,i)=>{(results[i+1].rows||[]).filter(r=>r.deletedAt||(r.status||"").toLowerCase()==="finalizado").forEach(r=>finished.push({module:m==="radio"?"Radio":m==="taxis"?"Taxis":m==="intercambiadores"?"Intercambiadores":"Home Ticket",title:r.spectacle||r.campaignName||r.position||"Registro",place:r.station||r.location||r.venue||"",from:r.startDate,to:r.endDate}))});
 app.innerHTML=pageHead("Archivo / Histórico","Cambios y campañas finalizadas")+(finished.length?'<section class="card" style="margin-bottom:14px"><div class="section-title"><h2>Campañas finalizadas / archivadas</h2><span class="badge">'+finished.length+'</span></div><div class="list">'+finished.map(r=>'<div class="item"><div><h3>'+esc(r.module)+' · '+esc(r.title)+'</h3><div class="item-meta"><span>'+esc(r.place)+'</span><span>'+fdate(r.from)+' → '+fdate(r.to)+'</span></div></div></div>').join("")+'</div></section>':'')+'<div class="card"><div class="section-title"><h2>Auditoría</h2></div><div class="table-wrap"><table><thead><tr><th>Fecha</th><th>Módulo</th><th>Acción</th><th>Elemento</th><th>Usuario</th></tr></thead><tbody>'+(d.rows||[]).map(r=>'<tr><td>'+new Date(r.at).toLocaleString("es-ES")+'</td><td>'+esc(r.module)+'</td><td>'+esc(r.action)+'</td><td>'+esc(r.elementId||"")+'</td><td>'+esc(r.actor?.email||"")+'</td></tr>').join("")+'</tbody></table></div></div>';
}

let calCursor=new Date();let calCursor=new Date();
