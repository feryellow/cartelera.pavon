...
function htChips(rows){const first=$(".ht-space");if(!first)return;const row=document.createElement("div");row.className="chip-row";row.style.marginBottom="6px";
 row.innerHTML='<button type="button" class="chip on" data-v="">Todos ('+rows.length+')</button>'+HOME_TICKET_SPACES.map(v=>'<button type="button" class="chip" data-v="'+esc(v)+'">'+esc(venueShort(v))+' ('+rows.filter(r=>r.venue===v).length+')</button>').join("");
