...
async function homeTicket(){
 ...
 const grouped=HOME_TICKET_SPACES.map(v=>'<div class="ht-space"><div class="section-title"><h2>'+esc(v)+'</h2><span class="badge">'+rows.filter(r=>r.venue===v).length+'</span></div><div class="list">'+htItems(rows.filter(r=>r.venue===v))+'</div></div>').join("");
